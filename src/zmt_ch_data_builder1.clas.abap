CLASS zmt_ch_data_builder1 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zmt_ch_data_builder1 IMPLEMENTATION.
...
*        output =
 ...
 METHOD fill_master_data.
   data : lt_bp type table of zmt_ch_bpa,
          lt_prod type table of zmt_ch_product.
   append value #(
  ...
  append value #(
   ...
   append value #(
                   ...
                   to lt_prod.
    insert zmt_ch_bpa from table @lt_bp.
    insert zmt_ch_product from table @lt_prod.
 ENDMETHOD.
 METHOD fill_transaction_data.
   data : o_rand type REF TO cl_abap_random_int,
          ...
          lv_ord_id type zmt_chr_eid,
          lt_so type table of zmt_ch_so_hdr,
          lt_so_i type table of zmt_ch_so_item.
   ...
   select * from zmt_ch_bpa into table @data(lt_bpa).
   select * from zmt_ch_product into table @data(lt_prod).
   do 50 times.
       ...
       enddo.
   enddo.
   insert zmt_ch_so_hdr from table @lt_so.
   insert zmt_ch_so_item from table @lt_so_i.
 ENDMETHOD.
 METHOD flush.
   delete from : zmt_ch_bpa, zmt_ch_product, zmt_ch_so_hdr, zmt_ch_so_item.
 ENDMETHOD.

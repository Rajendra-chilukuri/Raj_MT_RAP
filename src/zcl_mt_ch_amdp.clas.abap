CLASS zcl_mt_ch_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                                       value(i_bp_id) TYPE zmt_chr_eid
*                                    EXPORTING
*                                       VALUE(e_res) TYPE char40.
 CLASS-METHODS get_product_mrp IMPORTING
  ...
  CLASS-METHODS get_total_sales for table FUNCTION ZMT_CH_TF.
...
CLASS zcl_mt_ch_amdp IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

  zcl_mt_ch_amdp=>get_product_mrp(
     EXPORTING
       i_tax = 12
     IMPORTING
       otab  = data(itab) ).
...
*   get all the products in a implicit table (like a internal table in abap)
   lt_prod = select * from zmt_ch_product;
...
*   fill the otab for result (like in abap we fill another internal table with data)
  ...
  METHOD get_total_sales by DATABASE FUNCTION FOR HDB LANGUAGE SQLSCRIPT
                       OPTIONS READ-ONLY
                       USING zMT_CH_bpa zMT_CH_so_hdr zMT_CH_so_item
 .
   return select
           bpa.client,
           bpa.company_name,
    ...
    from zMT_CH_bpa as bpa
   INNER join zMT_CH_so_hdr as sls
   on bpa.bp_id = sls.buyer
   inner join zMT_CH_so_item as item
